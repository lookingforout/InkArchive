  ...
  const [user, setUser] = useState(null);
    ...
    useEffect(() => {
      ...
      const handleThreads = async () =>{
        try {
          setLoading(true);
          const threads = await fetch("http://localhost:5000/forum/threads/artdiscussion",{
            method: 'GET',
            headers: {
      ...
      <div className={styles.mainContent}>
        ...
        <div className={styles.forumContent}>
          <div className={styles.titleContainer}>
            <div className={styles.sectionTitle}>Art Discussion</div>
            {(user && user.role === 'guest') ? "" : <CreateThreadButton category="artdiscussion"/>}
          </div>
          <div>
            {loading ? (
