  ...
  const [user, setUser] = useState(null);
    ...
    useEffect(() => {
      ...
      const handleThreads = async () =>{
        try {
          setLoading(true);
          const threads = await fetch("http://localhost:5000/forum/threads/arttrading",{
            method: 'GET',
            headers: {
      ...
      <div className={styles.mainContent}>
        ...
        <div className={styles.forumContent}>
          <div className={styles.titleContainer}>
            <div className={styles.sectionTitle}>Art Trades</div>
            {(user && user.role === 'guest') ? "" : <CreateThreadButton category="arttrading"/>}
          </div>
          <div>
            {loading ? (
